...
```{r}
# Load in datasets
decennial_census10 <- read_csv("Class Files/Data/DECENNIALCD1162010.P1-Data.csv")
decennial_census20 <- read_csv("Class Files/Data/DECENNIALCD1182020.P1-Data.csv")

marijuana_laws2024 <- read_csv("Class Files/Data/marijuana-laws-by-state-2024.csv")
```
...
marijuana_laws2024_sf <- marijuana_laws2024 |>
  mutate(legality = ifelse(LegalStatus == "Mixed" & Medicinal == "Yes" & Decriminalized == "No", "Medicinal, not decriminalized", LegalStatus),
         legality = ifelse(LegalStatus == "Mixed" & Medicinal == "Yes" & Decriminalized == "Yes", "Medicinal, and decriminalized", legality),
         legality = ifelse(LegalStatus == "Mixed" & Medicinal == "CBD Oil Only" & Decriminalized == "No", "CBD Oil Only, not decriminalized", legality),
         legality = ifelse(LegalStatus == "Mixed" & Medicinal == "CBD Oil Only" & Decriminalized == "Yes", "CBD Oil Only, and decriminalized", legality),
         legality = ifelse(LegalStatus == "Legalized" & Decriminalized == "Yes", "Legalized, and decriminalized", legality),
         legality = ifelse(LegalStatus == "Legalized" & Decriminalized == "No", "Legalized, not decriminalized", legality),
         legality = ifelse(LegalStatus == "Fully Illegal" & Decriminalized == "No", "Fully Illegal, not decriminalized", legality),
         legality = ifelse(LegalStatus == "Fully Illegal" & Decriminalized == "Yes", "Fully Illegal, and decriminalized", legality)) |>
  rename(name = state) |>
...
factpal <- colorFactor(c("#e41a1c", "#377eb8", "#4daf4a", "#984ea3"),
                       levels(marijuana_laws2024_sf$LegalStatus))
...
leaflet(marijuana_laws2024_sf) |>
  ...
  addPolygons(
    ...
    labelOptions = labelOptions(
      ...
      textsize = "15px",
      direction = "auto")) |>
  addLegend(pal = factpal, values = ~marijuana_laws2024_sf$LegalStatus, 
