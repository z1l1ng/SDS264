...
>> It will choose those the rows and colums we want to count, then take the values of column in our pivot wider is a differetn way of organizing counts, we will have manufacturer, model, and then colums with the number of cylinders  
...
>> The function takes an input vector x and finds the difference between each element of x except
for the first and last elements. Then it adds the all of the differences together if the difference
is less than 0 and outputs a single number
